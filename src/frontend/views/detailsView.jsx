...
;
var generate_movie_view = function (plot, stars) {
return <div className="flex flex-col text-center items-center mt-4 bg-[#006466] h-full p-8 lg:px-16 m-2 lg:m-5 rounded-lg "><img src="https://designshack.net/wp-content/uploads/placehold.jpg" className="h-1/3 w-1/3"></img> <div className="text-left w-2/3 h-1/3 mt-4">Plot:  {plot}</div> <div className="h-1/3 mt-4">Stars:  {stars}</div></div>;
}
;
...
return <div className="flex justify-center h-screen lg:h-[600px] w-full"><div className="container justify-center w-full"><div className="flex flex-col items-center"><div className="flex flex-row items-center text-center mt-2"><div className="w-40 font-bold text-2xl lg:text-4xl lg:w-64">{title7} - {year8}</div> <Link to="trivia"><button className="ml-4 bg-[#4D194D] font-bold">Trivia</button></Link></div></div> <div className="flex flex-row flex-wrap container w-128 justify-center items-center text-center mt-2">{(some_QMARK_.call(null, episodes9)) ? (map.call(null, function (_PERCENT_1) {
return generate_view.call(null, _PERCENT_1);
}, episodes9)) : (null)} {(empty_QMARK_.call(null, episodes9)) ? (generate_movie_view.call(null, plot10, stars11)) : (null)}</div></div></div>;
}
;
