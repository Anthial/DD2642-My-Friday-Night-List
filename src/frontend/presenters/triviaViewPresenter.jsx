import {} from "../views/triviaView.jsx";
import {
  keyword,
  vector,
  arrayMap,
  ...
  js__GT_clj,
  nth,
} from "cherry-cljs/lib/cljs_core.js";
import { trivia_view } from "../views/triviaView.jsx";
import { fetchTrivia, fetchTestTenor } from "../../backend/api/imdb/IMDB.tsx";
...
var model = arrayMap(
  ...
  keyword("spoilerItems"),
  vector(),
  keyword("errorMessage"),
  ""
);
var id = "tt0118480";
var fetchTest = async function (query, API_KEY) {
  console.log(id);
  if (truth_(id !== null)) {
    let resp2 = await fetchTestTenor.call(null, query);
    console.log(resp2[0]);
    return (
      <img src={resp2[rand_int.call(null, 20)]["media"][0]["gif"]["url"]}></img>
    );
  }
};
var getResp = async function (set, id) {
  return set.call(null, await fetchTest.call(null, "cats", "aaa"));
};
var trivia_view_presenter = function (props) {
  let props6 = js__GT_clj.call(null, props);
  let vec__37 = useState.call(null, "init resp");
  let resp8 = nth.call(null, vec__37, 0, null);
  let setResp9 = nth.call(null, vec__37, 1, null);
  return (
    <div>
      <button
        onClick={function () {
          return getResp.call(null, setResp9, id);
        }}
      >
        click me to test fetching
      </button>{" "}
      {resp8} <img src="https://tenor.com/bPnKy.gif"></img>
    </div>
  );
};
var default$ = trivia_view_presenter;
export { model, id, fetchTest, getResp, trivia_view_presenter };
